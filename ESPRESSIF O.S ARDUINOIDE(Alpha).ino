...
// New Global Settings
bool cornerMode = true; // true = Rounded, false = Sharp
bool aeroMode = false;  // true = Glossy Aero Effect
...
int overlayX = 100, overlayY = 0;
bool overlayDragging = false;
int overlayDragStartX = 0, overlayDragStartY = 0;
unsigned long fpsLastFrameTime = 0, fpsDisplayTime = 0;
int fpsDisplay = 0, genFpsDisplay = 0, cpuDisplay = 0;
int frameCounter = 0, genFrameCounter = 0;
unsigned long cpuWorkAccum = 0;
void drawFpsOverlay() {
if (!overlayEnabled) return;
uint16_t bg = (currentTheme == 8) ? 0x1148 : themeTile;
fillRect(overlayX, overlayY, 120, 16, bg);
char buf[24];
snprintf(buf, sizeof(buf), "%d/%d %d%%", fpsDisplay, genFpsDisplay, cpuDisplay);
drawText5x7(overlayX + 2, overlayY + 4, buf, COLOR_GREEN);
}
void updateFpsCpu() {
...
// FIX: Add 5000 to round to the nearest integer before division.
// This prevents 99.5% - 99.9% from truncating down to 99%.
cpuDisplay = (cpuWorkAccum + 5000) / 10000; 
if (cpuDisplay > 100) cpuDisplay = 100;
cpuWorkAccum = 0;
...
drawText5x7(10, 108, "YOUR DEVICE RESTARTED", COLOR_WHITE); drawText5x7(10, 122, "DUE TO A POWER ISSUE.", COLOR_WHITE);
drawText5x7(10, 145, "ERR CODE 7: UNEXPECTED SHUTDOWN", COLOR_YELLOW); drawText5x7(10, 180, "TAP TOP BAR FOR RECOVERY OPTIONS", COLOR_WHITE);
for (int C = 0; C < 320; C++) {
...
if (EEPROM.read(7) == 1) { fillRoundRect(20, 210, 280, 20, cornerMode?3:0, COLOR_RED); drawText5x7(25, 215, "your device restarted due to a power issue", COLOR_WHITE); EEPROM.write(7, 0); EEPROM.commit(); }
...
bool cpu_metric = true; const int CPU_BOX_Y = 105, CPU_BOX_H = 26, CPU_TOGGLE_X = 245, CPU_TOGGLE_W = 55, OVERLAY_TOGGLE_X = 175, OVERLAY_TOGGLE_W = 60;
void os_drawCpuBox() { fillRect(10, CPU_BOX_Y, 300, CPU_BOX_H, themeTile); float c = readChipTempC(); float displayTemp = cpu_metric ? c : (c * 9.0f / 5.0f + 32.0f); uint32_t freq = getCpuFrequencyMhz(); char buf[40]; snprintf(buf, sizeof(buf), "%.1f%s  %luMHZ", displayTemp, cpu_metric ? "C" : "F", (unsigned long)freq); drawText5x7(16, CPU_BOX_Y + 9, buf, themeText); fillRoundRect(OVERLAY_TOGGLE_X, CPU_BOX_Y + 2, OVERLAY_TOGGLE_W, CPU_BOX_H - 4, cornerMode?3:0, COLOR_GRAY); drawText5x7(OVERLAY_TOGGLE_X + 5, CPU_BOX_Y + 9, overlayEnabled ? "OVR:ON" : "OVR:OFF", COLOR_WHITE); fillRoundRect(CPU_TOGGLE_X, CPU_BOX_Y + 2, CPU_TOGGLE_W, CPU_BOX_H - 4, cornerMode?3:0, COLOR_GRAY); drawText5x7(CPU_TOGGLE_X + 10, CPU_BOX_Y + 9, cpu_metric ? "->F" : "->C", COLOR_WHITE); }
...
fillRoundRect(10, 45, 300, 26, cornerMode?3:0, COLOR_GRAY); drawText5x7(15, 53, "THEMES", COLOR_WHITE);
fillRoundRect(10, 75, 300, 26, cornerMode?3:0, COLOR_GRAY); drawText5x7(15, 83, "HEATER TEMP RANGE", COLOR_WHITE);
os_drawCpuBox();
fillRoundRect(10, 135, 300, 26, cornerMode?3:0, COLOR_GRAY);
char pinLabel[24]; snprintf(pinLabel, sizeof(pinLabel), "PIN LOCK: %s", pinEnabled ? "ON" : "OFF");
drawText5x7(15, 143, pinLabel, COLOR_WHITE);
// New Aero Toggle
fillRoundRect(10, 165, 300, 26, cornerMode?3:0, aeroMode ? COLOR_CYAN : COLOR_GRAY);
drawText5x7(15, 173, aeroMode ? "AERO MODE: ON" : "AERO MODE: OFF", COLOR_WHITE);
// Corner Mode Toggle
fillRoundRect(10, 195, 300, 26, cornerMode?3:0, cornerMode ? COLOR_CYAN : COLOR_GRAY);
drawText5x7(15, 203, cornerMode ? "CORNER MODE: ROUNDED" : "CORNER MODE: SHARP", COLOR_WHITE);
fillRoundRect(10, 225, 145, 26, cornerMode?3:0, COLOR_GRAY); drawText5x7(20, 233, "WIN HISTORY", COLOR_WHITE);
fillRoundRect(165, 225, 145, 26, cornerMode?3:0, COLOR_GRAY); drawText5x7(200, 233, "DONE", COLOR_WHITE);
}
void os_drawThemes() {
...
if (isHomeTouched(x, y)) { globalState = 0; return; }
// Handle continuous dragging for wheel and slider
...
// Button Clicks (Only if not dragging)
if (!ct_wheelDragging && !ct_sliderDragging) {
if (x >= 170 && x <= 300) {
...
if (os_uiState == 0) {
if (y >= 45 && y <= 71) { os_uiState = 1; os_drawThemes(); return; }
if (y >= 75 && y <= 101) { globalState = 11; temp_reset(); return; }
...
// New Toggles in Main Menu
if (y >= 165 && y <= 191) { aeroMode = !aeroMode; os_drawMain(); return; }
if (y >= 195 && y <= 221) { cornerMode = !cornerMode; os_drawMain(); return; }
if (y >= 135 && y <= 161) {
...
else { os_pinLen = 0; memset(os_pinBuf, 0, sizeof(os_pinBuf)); os_drawPinPad("WRONG PIN - RETRY"); }
}
}
}
}
...
bool heaterActive = false, condensationWarned = false; unsigned long heaterLastCheck = 0; float heaterLastTempF = 0;
void heater_update() { unsigned long now = millis(); if (now - heaterLastCheck < HEATER_CHECK_MS) return; heaterLastCheck = now; heaterLastTempF = heater_readTempF(); float targetF = (heaterLowF + heaterMaxF) / 2.0f; if (heaterLastTempF >= heaterMaxF) { heaterActive = false; } else if (heaterLastTempF < heaterLowF) { heaterActive = true; } else if (heaterLastTempF >= targetF) { heaterActive = false; } }
void heater_burn() { if (!heaterActive) return; volatile double x = 0; for (int i = 0; i < 4000; i++) { x += sin((double)i) * cos((double)i) * sqrt((double)i + 1.0); } }
int fw_frameCount = 0;
...
configTime(0, 0, "pool.ntp.org", "time.nist.gov"); // NTP Sync initialization
delay(200);
...
// Setup PWM for Serial 1 & 2
ledcAttach(21, 5000, 8); // Red / Serial 1 LED
ledcAttach(22, 5000, 8); // Green
ledcAttach(26, 5000, 8); // Blue (GPIO 35 is input-only, so 26 is used)
...
overlayEnabled = prefs.getBool("overlayEnabled", true); overlayX = prefs.getInt("overlayX", 100); overlayY = prefs.getInt("overlayY", 0);
...
int16_t x, y;
if (readTouch(x, y)) {
if (overlayDragging) { overlayX = x - overlayDragStartX; overlayY = y - overlayDragStartY; if (overlayX < 0) overlayX = 0; if (overlayX > 200) overlayX = 200; if (overlayY < 0) overlayY = 0; if (overlayY > 224) overlayY = 224; drawFpsOverlay(); global_wasTouched = true; }
else if (overlayEnabled && x >= overlayX && x <= overlayX + 120 && y >= overlayY && y <= overlayY + 16) { overlayDragging = true; overlayDragStartX = x - overlayX; overlayDragStartY = y - overlayY; global_wasTouched = true; }
else {
...
// Reset dragging states when touch is released
if (ct_wheelDragging || ct_sliderDragging) {
...
if (globalState == 7 && os_uiState == 0 && millis() - cpuBoxLastUpdate > 1000) { cpuBoxLastUpdate = millis(); os_drawCpuBox(); }
heater_update();
