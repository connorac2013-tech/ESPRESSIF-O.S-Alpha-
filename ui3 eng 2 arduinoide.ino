...
int tftFormatIdx = 0;
int ledFormatIdx = 0;
int os_prevSubmenu = 0;
int timeZoneOffset = 0; // NEW: Timezone offset in hours
const char* colorFormats[] = {"RGB", "RBG", "GRB", "GBR", "BRG", "BGR"};
...
// FIX 2: Safe TFT format re-application without crashing SPI
void applyTftFormat() {
    uint8_t madctl = 0xA0;
    if (tftFormatIdx % 2 != 0) madctl |= 0x08;
    tft_cmd(0x36);
    tft_data(madctl);
}
...
delay(50);
tft_cmd(0x01); delay(120);
uint8_t madctl = 0xA0;
if (tftFormatIdx % 2 != 0) madctl |= 0x08;
tft_cmd(0x36); tft_data(madctl);
...
fillRect(cx-8, cy-8, 6, 6, COLOR_RED);
fillRect(cx+2, cy-8, 6, 6, COLOR_GREEN);
fillRect(cx-3, cy+2, 6, 6, COLOR_BLUE);
} else if (iconType == 13) {
...
drawTile(0, 2, COLOR_PURPLE, "Calc", 10); drawTile(1, 2, COLOR_ORANGE, "Port 1", 11); drawTile(2, 2, COLOR_LIGHT_ORANGE, "RGB", 12); drawTile(3, 2, COLOR_PINK, "WinLog", 13); drawTile(4, 2, COLOR_RED, "Power", 14);
...
if (connected) {
    // FIX 6: Re-sync NTP time now that Wi-Fi is connected
    configTime(timeZoneOffset * 3600, 0, "pool.ntp.org", "time.nist.gov");
    struct tm timeinfo;
    getLocalTime(&timeinfo, 5000); // Force sync
...
int os_uiState = 0, os_themeScroll = 0, os_submenuState = 0;
char os_pinBuf[5] = "", os_pinFirst[5] = ""; int os_pinLen = 0;
...
// FIX 4: Themes Submenu with Aero and Round Corners
void drawThemeSelector() {
fillScreenTheme();
fillRect(0, 0, 320, 40, themeAccent); drawText5x7(45, 15, "SELECT THEME", COLOR_WHITE); drawHomeButton(); drawMoreButton();

int startX = 10, startY = 50, tileW = 145, tileH = 40, gapX = 10, gapY = 10;
for (int i = 0; i < 6; i++) { 
    int idx = os_themeScroll + i;
    if (idx >= 10) break;
    int col = i % 2;
    int row = i / 2;
    int bx = startX + col * (tileW + gapX);
    int by = startY + row * (tileH + gapY);
    
    drawGlossyTileCustom(bx, by, tileW, tileH);
    
    if (idx == currentTheme) {
        drawLine(bx, by, bx + tileW - 1, by, COLOR_YELLOW);
        drawLine(bx, by + tileH - 1, bx + tileW - 1, by + tileH - 1, COLOR_YELLOW);
        drawLine(bx, by, bx, by + tileH - 1, COLOR_YELLOW);
        drawLine(bx + tileW - 1, by, bx + tileW - 1, by + tileH - 1, COLOR_YELLOW);
    }
    
    fillRect(bx + 5, by + 5, 30, 30, os_themeSwatches[idx]);
    drawText5x7(bx + 40, by + 12, os_themeNames[idx], (idx == currentTheme) ? COLOR_YELLOW : themeText);
}

drawGlossyTileCustom(10, 200, 300, 25);
drawText5x7(130, 207, "NEXT / PREV", themeText);
}

void drawFormatSelector(bool isTft) {
fillScreenTheme();
fillRect(0, 0, 320, 40, themeAccent);
drawText5x7(45, 15, isTft ? "TFT FORMAT" : "LED FORMAT", COLOR_WHITE);
drawHomeButton(); drawMoreButton();
int startX = 10, startY = 60, tileW = 145, tileH = 40, gapX = 10, gapY = 10;
int* currentIdx = isTft ? &tftFormatIdx : &ledFormatIdx;
for (int i = 0; i < 6; i++) {
int col = i % 2;
int row = i / 2;
int bx = startX + col * (tileW + gapX);
int by = startY + row * (tileH + gapY);
uint16_t btnColor = (i == *currentIdx) ? COLOR_GREEN : COLOR_GRAY;
fillRoundRect(bx, by, tileW, tileH, cornerMode?3:0, btnColor);
drawText5x7(bx + 50, by + 12, colorFormats[i], COLOR_WHITE);
}
fillRoundRect(10, 220, 300, 20, cornerMode?3:0, COLOR_GRAY);
drawText5x7(140, 223, "BACK", COLOR_WHITE);
}

// FIX 3 & 5: Consistent Button Sizes & Time Zone Settings
void os_drawMain() {
if (os_uiState == 8) { drawThemeSelector(); return; }
if (os_uiState == 9) { drawFormatSelector(false); return; }
if (os_uiState == 10) { drawFormatSelector(true); return; }
if (os_uiState == 6) { 
    // Time Zone UI
    fillScreenTheme();
    fillRect(0, 0, 320, 40, themeAccent); drawText5x7(45, 15, "TIME ZONE", COLOR_WHITE);
    drawHomeButton(); drawMoreButton();
    
    drawGlossyTileCustom(10, 60, 300, 40);
    char buf[32]; snprintf(buf, sizeof(buf), "OFFSET: %+d HOURS", timeZoneOffset);
    drawText5x7(80, 75, buf, themeText);
    
    drawGlossyTileCustom(10, 120, 140, 40);
    drawText5x7(50, 135, "- 1 HOUR", themeText);
    
    drawGlossyTileCustom(170, 120, 140, 40);
    drawText5x7(210, 135, "+ 1 HOUR", themeText);
    
    drawGlossyTileCustom(10, 200, 300, 30);
    drawText5x7(120, 210, "BACK & SAVE", themeText);
    return; 
}

fillScreenTheme();
drawHomeButton(); drawMoreButton();
if (os_submenuState == 0) {
fillRect(0, 0, 320, 40, themeAccent); drawText5x7(45, 15, "SYSTEM SETTINGS", COLOR_WHITE);

drawGlossyTileCustom(10, 45, 300, 30); drawText5x7(15, 55, "THEMES", themeText);
drawGlossyTileCustom(10, 85, 300, 30); drawText5x7(15, 95, "HARDWARE SETUP", themeText);
drawGlossyTileCustom(10, 125, 300, 30); drawText5x7(15, 135, "POWER & PERFORMANCE", themeText);
drawGlossyTileCustom(10, 165, 300, 30); drawText5x7(15, 175, "PRIVACY & SECURITY", themeText);

// FIX 3: Extended background to cover the switch
drawGlossyTileCustom(10, 205, 300, 30); 
drawText5x7(15, 215, overlayEnabled ? "OVERLAY: ON" : "OVERLAY: OFF", themeText);
drawToggleSwitch(265, 213, overlayEnabled);

drawGlossyTileCustom(10, 245, 145, 20); drawText5x7(20, 250, "WIN HIST", themeText);
drawGlossyTileCustom(165, 245, 145, 20); drawText5x7(200, 250, "DONE", themeText);
} else if (os_submenuState == 1) {
fillRect(0, 0, 320, 40, themeAccent); drawText5x7(45, 15, "POWER & PERF", COLOR_WHITE);
drawGlossyTileCustom(10, 45, 300, 22); drawText5x7(15, 51, "HEATER TEMP RANGE", themeText);
drawGlossyTileCustom(10, 70, 300, 22);
drawText5x7(15, 76, powerSaverEnabled ? "POWER SAVER: ON (50MHZ)" : "POWER SAVER: OFF (240MHZ)", themeText);
drawToggleSwitch(260, 73, powerSaverEnabled);
os_drawCpuBox();
drawGlossyTileCustom(10, 220, 300, 20); drawText5x7(140, 225, "BACK", themeText);
} else if (os_submenuState == 2) {
fillRect(0, 0, 320, 40, themeAccent); drawText5x7(45, 15, "PRIVACY & SEC", COLOR_WHITE);
drawGlossyTileCustom(10, 45, 300, 30);
char pinLabel[24]; snprintf(pinLabel, sizeof(pinLabel), "PIN LOCK: %s", pinEnabled ? "ON" : "OFF");
drawText5x7(15, 55, pinLabel, themeText);

// FIX 5: Time Zone Button
drawGlossyTileCustom(10, 85, 300, 30);
char tzBuf[32]; snprintf(tzBuf, sizeof(tzBuf), "TIME ZONE: %+d HOURS", timeZoneOffset);
drawText5x7(15, 95, tzBuf, themeText);

drawGlossyTileCustom(10, 220, 300, 20); drawText5x7(140, 225, "BACK", themeText);
} else if (os_submenuState == 3) {
fillRect(0, 0, 320, 40, themeAccent); drawText5x7(45, 15, "HARDWARE SETUP", COLOR_WHITE);
drawGlossyTileCustom(10, 45, 300, 30); drawText5x7(15, 53, "TFT RGB FORMAT", themeText);
drawGlossyTileCustom(10, 85, 300, 30);
char rgbLabel[32]; snprintf(rgbLabel, sizeof(rgbLabel), "LED RGB FORMAT: %s", colorFormats[ledFormatIdx]);
drawText5x7(15, 93, rgbLabel, themeText);
drawGlossyTileCustom(10, 220, 300, 20); drawText5x7(140, 225, "BACK", themeText);
}
}
...
int ct_target = 0;
float ct_hue = 0, ct_sat = 1.0, ct_val = 1.0;
...
// FIX 1: Added yield() to prevent Task Watchdog resets during heavy drawing
...
bool s1_state = false;
void serial1_reset() {
fillScreenTheme(); fillRect(0, 0, 320, 40, themeAccent); drawText5x7(80, 15, "PORT 1", COLOR_WHITE); drawHomeButton(); drawMoreButton();
...
float rgb_hue = 0, rgb_sat = 1.0, rgb_val = 1.0;
...
uint8_t g8 = ((c >> 5) & 0x3F) * 255 / 63;
uint8_t b8 = (c & 0x1F) * 255 / 31;
uint8_t pRed = r8, pGreen = g8, pBlue = b8;
switch(ledFormatIdx) {
case 0: pRed=r8; pGreen=g8; pBlue=b8; break;
case 1: pRed=g8; pGreen=b8; pBlue=r8; break;
case 2: pRed=b8; pGreen=r8; pBlue=g8; break;
case 3: pRed=r8; pGreen=b8; pBlue=g8; break;
case 4: pRed=g8; pGreen=r8; pBlue=b8; break;
case 5: pRed=b8; pGreen=g8; pBlue=r8; break;
}
ledcWrite(RGB_RED, 255 - pRed);
ledcWrite(RGB_GREEN, 255 - pGreen);
ledcWrite(RGB_BLUE, 255 - pBlue);
}

// FIX 8: Added predefined RGB color buttons
...
if (py % 10 == 0) yield(); // Prevent watchdog
}
...
// Predefined Colors
int btnW = 45, btnH = 20, btnY = 180;
uint16_t colors[6] = {COLOR_RED, COLOR_YELLOW, COLOR_BLUE, COLOR_PURPLE, COLOR_GREEN, COLOR_WHITE};
const char* labels[6] = {"R", "Y", "B", "P", "G", "W"};
for(int i=0; i<6; i++) {
    int bx = 10 + i * (btnW + 5);
    fillRoundRect(bx, btnY, btnW, btnH, cornerMode?3:0, colors[i]);
    drawText5x7(bx + 18, btnY + 6, labels[i], COLOR_BLACK);
}
...
fillRect(10 + valPos - 2, sliderY - 2, 4, 18, COLOR_WHITE);
fillRoundRect(10, 225, 300, 20, cornerMode?3:0, COLOR_RED);
drawText5x7(140, 230, "TURN OFF LED", COLOR_WHITE);
}
...
// FIX 8: Touch logic for predefined colors
if (y >= 180 && y <= 200) {
    int idx = (x - 10) / 50;
    if (idx >= 0 && idx < 6) {
        switch(idx) {
            case 0: rgb_hue = 0; rgb_sat = 1; rgb_val = 1; break; // R
            case 1: rgb_hue = 60; rgb_sat = 1; rgb_val = 1; break; // Y
            case 2: rgb_hue = 240; rgb_sat = 1; rgb_val = 1; break; // B
            case 3: rgb_hue = 300; rgb_sat = 1; rgb_val = 1; break; // P
            case 4: rgb_hue = 120; rgb_sat = 1; rgb_val = 1; break; // G
            case 5: rgb_hue = 0; rgb_sat = 0; rgb_val = 1; break; // W
        }
        rgb_updatePins();
        serial2_drawRGB();
        return;
...
void os_reset() { os_uiState = 0; os_themeScroll = (currentTheme > 4) ? currentTheme - 4 : 0; os_submenuState = 0; os_drawMain(); }
void os_touch(int16_t x, int16_t y) {
if (os_uiState == 8) {
// Theme Selector Touch
if (y >= 200 && y <= 225 && x >= 10 && x <= 310) { 
    os_themeScroll += 2; 
    if(os_themeScroll > 8) os_themeScroll = 0;
    drawThemeSelector(); 
    return; 
}
int startX = 10, startY = 50, tileW = 145, tileH = 40, gapX = 10, gapY = 10;
...
applyTheme(idx); os_persistTheme();
drawThemeSelector();
return;
}
}
...
if (os_uiState == 9 || os_uiState == 10) {
// Format Selector Touch
bool isTft = (os_uiState == 10);
int* currentIdx = isTft ? &tftFormatIdx : &ledFormatIdx;
int startX = 10, startY = 60, tileW = 145, tileH = 40, gapX = 10, gapY = 10;
for (int i = 0; i < 6; i++) {
int col = i % 2;
...
*currentIdx = i;
prefs.begin("sysconfig", false);
prefs.putInt(isTft ? "tftFormat" : "ledFormat", i);
prefs.end();
// FIX 2: Safely apply format without re-initializing SPI
if (isTft) applyTftFormat(); 
drawFormatSelector(isTft);
return;
}
}
if (y >= 220 && y <= 240 && x >= 10 && x <= 310) {
os_uiState = 0; os_submenuState = os_prevSubmenu; os_drawMain();
return;
}
return;
}

// FIX 5: Time Zone Touch Logic
if (os_uiState == 6) {
    if (y >= 120 && y <= 160) {
        if (x >= 10 && x <= 150) { timeZoneOffset--; if(timeZoneOffset < -12) timeZoneOffset = -12; os_drawMain(); }
        else if (x >= 170 && x <= 310) { timeZoneOffset++; if(timeZoneOffset > 14) timeZoneOffset = 14; os_drawMain(); }
    }
    if (y >= 200 && y <= 230) {
        prefs.begin("sysconfig", false); prefs.putInt("tzOffset", timeZoneOffset); prefs.end();
        configTime(timeZoneOffset * 3600, 0, "pool.ntp.org", "time.nist.gov");
        os_submenuState = 2; os_uiState = 0; os_drawMain();
    }
    return;
}

if (os_submenuState == 0) {
if (y >= 45 && y <= 75 && x >= 10 && x <= 310) { os_prevSubmenu = 0; os_uiState = 8; drawThemeSelector(); return; }
if (y >= 85 && y <= 115 && x >= 10 && x <= 310) { os_prevSubmenu = 0; os_submenuState = 3; os_drawMain(); return; }
if (y >= 125 && y <= 155 && x >= 10 && x <= 310) { os_prevSubmenu = 0; os_submenuState = 1; os_drawMain(); return; }
if (y >= 165 && y <= 195 && x >= 10 && x <= 310) { os_prevSubmenu = 0; os_submenuState = 2; os_drawMain(); return; }
if (y >= 205 && y <= 235 && x >= 10 && x <= 310) {
if (isToggleTouched(x, y, 265, 213)) {
overlayEnabled = !overlayEnabled;
prefs.begin("sysconfig", false); prefs.putBool("overlayEnabled", overlayEnabled); prefs.end();
os_drawMain();
}
return;
}
...
if (os_submenuState == 1) {
if (y >= 45 && y <= 67 && x >= 10 && x <= 310) { globalState = 11; temp_reset(); return; }
if (isToggleTouched(x, y, 260, 73)) {
powerSaverEnabled = !powerSaverEnabled;
prefs.begin("sysconfig", false); prefs.putBool("powerSaver", powerSaverEnabled); prefs.end();
if (powerSaverEnabled) setCpuFrequencyMhz(50); else setCpuFrequencyMhz(240);
os_drawMain(); return;
}
if (y >= 220 && y <= 240 && x >= 10 && x <= 310) { os_submenuState = 0; os_drawMain(); return; }
return;
}
if (os_submenuState == 2) {
if (y >= 45 && y <= 75 && x >= 10 && x <= 310) {
...
// FIX 5: Time Zone Button Touch
if (y >= 85 && y <= 115 && x >= 10 && x <= 310) { os_uiState = 6; os_drawMain(); return; }

if (y >= 220 && y <= 240 && x >= 10 && x <= 310) { os_submenuState = 0; os_drawMain(); return; }
return;
}
if (os_submenuState == 3) {
if (y >= 45 && y <= 75 && x >= 10 && x <= 310) { os_prevSubmenu = 3; os_uiState = 10; drawFormatSelector(true); return; }
if (y >= 85 && y <= 115 && x >= 10 && x <= 310) { os_prevSubmenu = 3; os_uiState = 9; drawFormatSelector(false); return; }
if (y >= 220 && y <= 240 && x >= 10 && x <= 310) { os_submenuState = 0; os_drawMain(); return; }
return;
}
if (os_uiState == 2 || os_uiState == 3 || os_uiState == 4) {
if (y >= 80 && y < 80 + 4 * 31) {
...
os_updatePinDots();
if (os_pinLen == 4) {
if (strcmp(os_pinBuf, os_pinFirst) == 0) { pinEnabled = true; strcpy(pinCode, os_pinBuf); os_persistPin(); os_uiState = 0; os_submenuState = 2; os_drawMain(); }
else { os_pinLen = 0; memset(os_pinBuf, 0, sizeof(os_pinBuf)); os_uiState = 2; os_drawPinPad("DIDN'T MATCH - RETRY"); }
}
} else if (os_uiState == 4) {
os_updatePinDots();
if (os_pinLen == 4) {
if (strcmp(os_pinBuf, pinCode) == 0) { pinEnabled = false; os_persistPin(); os_uiState = 0; os_submenuState = 2; os_drawMain(); }
...
if (y >= 210 && y <= 236 && x >= 10 && x <= 310) { os_uiState = 0; os_submenuState = 1; os_drawMain(); return; }
return;
}
if (os_uiState == 5) {
...
// FIX 6: Increased timeout for NTP sync
void clock_draw() {
fillScreenTheme();
fillRect(0, 0, 320, 40, themeAccent);
...
drawMoreButton();
struct tm timeinfo;
bool timeSynced = getLocalTime(&timeinfo, 2000); 
if (!timeSynced) {
...
// FIX 7: Safe string drawing without memory fragmentation
void term_drawCmd() {
    char dispBuf[64];
    snprintf(dispBuf, sizeof(dispBuf), "> %s_", term_cmd.c_str());
    fillRect(10, 100, 300, 20, COLOR_BLACK);
    drawText5x7(15, 110, dispBuf, COLOR_GREEN);
}
...
fillRect(10, 50, 300, 100, COLOR_BLACK);
char cmdBuf[64];
snprintf(cmdBuf, sizeof(cmdBuf), "> %s", term_cmd.c_str());
drawText5x7(15, 60, cmdBuf, COLOR_GREEN);
...
if (col >= 0 && col < count) { 
    term_cmd += (char)(keys[row][col] + 32); 
    term_drawCmd(); 
}
} else {
if (x < 105) { term_cmd += " "; term_drawCmd(); }
else if (x < 210) term_execute();
else { if(term_cmd.length() > 0) term_cmd.remove(term_cmd.length()-1); term_drawCmd(); }
}
}
}
...
// FIX 5 & 6: Load timezone and init NTP
...
prefs.begin("sysconfig", true);
tftFormatIdx = prefs.getInt("tftFormat", 0);
ledFormatIdx = prefs.getInt("ledFormat", 0);
prefs.end();
ili9341_init(); drawSplashScreen(); delay(1800);
ledcAttach(21, 5000, 8);
ledcAttach(22, 5000, 8);
ledcAttach(SOUND_PIN, 2000, 8);
ledcAttach(RGB_RED, 5000, 8);
ledcAttach(RGB_GREEN, 5000, 8);
ledcAttach(RGB_BLUE, 5000, 8);
pinMode(LIGHT_SENSOR_PIN, INPUT);
ledcWrite(RGB_RED, 255);
ledcWrite(RGB_GREEN, 255);
ledcWrite(RGB_BLUE, 255);
prefs.begin("sysconfig", true);
...
cpu_metric = prefs.getBool("cpu_metric", true);
...
int newBright = map(sensorVal, 0, 4095, 12, 255);
if (abs(newBright - brightness) > 5) {
...
if (globalState == 7 && os_submenuState == 1 && millis() - cpuBoxLastUpdate > cpuUpdateInterval) {
cpuBoxLastUpdate = millis();
os_drawCpuBox();
}
heater_update();
if (!condensationWarned && heaterLastCheck > 0 && heaterLastTempF < (float)(heaterLowF - 20)) { condensationWarned = true; condensation_warn(); }
// heater_burn(); // Handled in cpuTask now
